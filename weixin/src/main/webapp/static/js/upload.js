...
function initUploaders(bucket, domain, inputName) {
    var uploader = new plupload.Uploader({
        ...
        flash_swf_url: domain + 'js/plugins/plupload-2.1.2/js/Moxie.swf',
        silverlight_xap_url: domain + 'js/plugins/plupload-2.1.2/js/Moxie.xap',
...
function initUploaders2(bucket, domain) {
    var uploader = new plupload.Uploader({
        runtimes: 'html5,flash,silverlight,html4',
        ...
        init: {
            ...
            FileUploaded: function () {
                $("#uploaderInput2").before('<li class="weui-uploader__file certImageUpload"'
                    + 'style="display:none;background-image:url(' +
                    'http://' + bucket + '.img-cn-shanghai.aliyuncs.com/' + g_object_name + '?x-oss-process=image/resize,m_fill,h_100,w_100'
                    + ')"><input name="certImage" type="hidden" value="' + g_object_name + '" ></li>');
                $('.certificate-text2').html("已上传").attr('id', 'haveUpload')
...
function initUploaders3(bucket, domain, inputName) {
    var uploader = new plupload.Uploader({
        runtimes: 'html5,flash,silverlight,html4',
        ...
        init: {
            ...
            FileUploaded: function () {
                $("#uploaderInput3").before('<li class="weui-uploader__file valueImgs"'
                    + 'style="background-image:url(' +
                    'http://' + bucket + '.img-cn-shanghai.aliyuncs.com/' + g_object_name + '?x-oss-process=image/resize,m_fill,h_100,w_100'
                    + ')"><input name="' + inputName + '" type="hidden" value="' + g_object_name + '" ></li>');
            }
